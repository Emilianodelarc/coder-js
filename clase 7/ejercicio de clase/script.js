...
console.log(myCarrito2);
    ...
    function total(){
        ...
        for (var i of items){
            ...
            var totalCarritos = items.reduce((acc, el) => acc + el, 0);
            var myCarritoJSON = JSON.stringify(myCarrito);
            var myCarrito2JSON = JSON.stringify(myCarrito2);
            var totalListJSON =JSON.stringify(totalCarritos);

            sessionStorage.setItem("carrito_1_" + nombreUsuario, myCarritoJSON);
            sessionStorage.setItem("carrito_2_" + nombreUsuario, myCarrito2JSON);
            sessionStorage.setItem(" su total es  $ " + totalListJSON, nombreUsuario);

            console.log("carrito_1_" + nombreUsuario, myCarritoJSON);
            console.log("carrito_2_" + nombreUsuario, myCarrito2JSON);